...
def process_omr_sheet(image_path,Sno,posScore,negScore,unattemptScore, threshold, answerKey):
    ...
    for cnt in contours:
        ...
        if area>120 and area<250 and aspectRatio > 1.2:
            ...
            limit=0.8*total # checking if max area is covered , because if circle is identified as rectangle it will not cover entire area
            if total-area>23.5:
    ...
    for rect in rects:
        if rect[0]>=minX-2 and rect[0]<=minX+2:
            col.append(rect)
            cv2.circle(responseROI,(rect[0],rect[1]),3,(0,165,255),cv2.FILLED)
    ...
    print(Sno,regNo,Set,schoolCode,correctAns,IncorrectAns,Left,score)
    return [Sno,regNo,Set,schoolCode,correctAns,IncorrectAns,Left,score],flattened_data,mainImage,responseROI
